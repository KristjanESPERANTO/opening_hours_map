...
function createMap() {
  ...
  window.useUserKey = function (key) {
    if (related_tags.indexOf(key) === -1) {
      /* Add the new key to related_tags. */
  ...
  const hasViewInUrl = typeof params.lat === "string" || typeof params.lon === "string" || typeof params.zoom === "string";
  ...
  if (typeof params["tags"] != "string") {
    switch (OHMode) {
      case 2:
        OSM_tags.push("collection_times");
        break;
      default:
        OSM_tags.push("opening_hours");
        break;
    }
  } else {
  ...
  map.addControl(
    new OpenLayers.Control.LoadStatus({
  ...
  map.events.register("moveend", map, function () {
    ...
    const currentCenterWgs84 = currentCenter.clone().transform(map.getProjectionObject(), new OpenLayers.Projection("EPSG:4326"));
    const mapViewToSave = {
  ...
  /* {{{ Base layers */
  map.addLayer(new OpenLayers.Layer.OSM("Mapnik", "https://tile.openstreetmap.org/${z}/${x}/${y}.png", { numZoomLevels: 19 }));
  map.addLayer(
    new OpenLayers.Layer.OSM("CyclOSM", "https://a.tile-cyclosm.openstreetmap.fr/cyclosm/${z}/${x}/${y}.png", {
      numZoomLevels: 20,
  ...
  /* {{{ opening_hours layer */
  map.addLayer(
    (poi_layer = new OpenLayers.Layer.PopupMarker("opening hours like values (opening_hours, lit, …)", {
      minZoom: 11,
      ...
      createHtmlFromData: function (data) {
        ...
        if (data._oh_state == "error" || data._oh_state == "na") {
          ...
          if (warnings.length > 0) text += `<br/>Warnings: <div class="v">${warnings.join("<br/>\n")}</div>`;
      ...
      getIconUrl: function (data) {
        if (this.evaluateOH(data) != "na") {
          switch (data._oh_state) {
            ...
            case "warning":
              return `img/circle_${
                data._it_object.getState() ? "green" : data._it_object.getUnknown() ? "yellow" : "red"
      ...
      overpassQL: function (keyvalues) {
        ...
        const bbox = this.map.getExtent().transform(this.map.getProjectionObject(), this.map.displayProjection);

        if (Object.keys(nominatim_data_global).length === 0) {
          this.updateNominatimData(bbox.top, bbox.left);
        }

        const bboxQuery = OpenLayers.String.format("[bbox:${bottom},${left},${top},${right}]", bbox);
      ...
      updateNominatimData: function (lat, lon) {
        reverseGeocodeLocation(lat, lon, function (nominatim_data) {
      ...
      jsonCallback: function (data) {
        ...
        if (typeof elements[0] !== "undefined") {
          ...
          if (
            ...
            (typeof this.lastLat === "undefined" || Math.abs(this.lastLat - firstLat) > 2 || Math.abs(this.lastLon - firstLon) > 2)
          ) {
      ...
      filter: function (data) {
        ...
        const hasWarnings = !hasError && oh && typeof oh.getWarnings === "function" ? oh.getWarnings().length > 0 : false;
        const canIterate = !hasError && it && typeof it.getState === "function" && typeof it.getUnknown === "function";
  ...
  if (!map.getCenter()) {
      ...
      try {
        ...
        if (savedMapViewRaw) {
          ...
          if (Number.isFinite(savedLon) && Number.isFinite(savedLat) && Number.isFinite(savedZoom)) {
            const savedCenter = new OpenLayers.LonLat(savedLon, savedLat).transform(
    ...
    if (!restoredSavedView) {
      map.zoomToExtent(
        new OpenLayers.Bounds(7.1042, 50.7362, 7.1171, 50.7417).transform(new OpenLayers.Projection("EPSG:4326"), map.getProjectionObject()),
      );
    }
  }
  /* }}} */
...
function initializeUI() {
  ...
  headerHTML +=
    '<button id="theme-toggle" style="padding: 0.5em 1em; cursor: pointer; border: 1px solid #ccc; background: transparent; border-radius: 4px;" title="Toggle dark mode">🌓</button>';
  ...
  geolocateBtn.addEventListener("click", function () {
    ...
    navigator.geolocation.getCurrentPosition(
      function (position) {
        ...
        const point = new OpenLayers.LonLat(lon, lat).transform(new OpenLayers.Projection("EPSG:4326"), map.getProjectionObject());
        ...
        if (typeof position.coords.accuracy === "number" && position.coords.accuracy > 0) {
          const accuracyGeometry = OpenLayers.Geometry.Polygon.createRegularPolygon(positionGeometry, position.coords.accuracy, 40, 0);
